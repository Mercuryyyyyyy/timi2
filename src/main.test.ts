...
describe('integration: hero chain integrity', () => {
  it('all 10 heroes have valid properties', () => {
  ...
  it('merge chain scoring is correct end-to-end', () => {
    ...
    expect(getHeroByTier(10)!.score).toBe(2560);
  ...
  it('hero IDs match expected values from API', () => {
    ...
    expect(HERO_CHAIN[1].heroId).toBe(182);  // 干将莫邪
    expect(HERO_CHAIN[2].heroId).toBe(193);  // 铠
    expect(HERO_CHAIN[3].heroId).toBe(127);  // 甄姬
    expect(HERO_CHAIN[4].heroId).toBe(538);  // 云缨
    expect(HERO_CHAIN[5].heroId).toBe(563);  // 海诺
    expect(HERO_CHAIN[6].heroId).toBe(174);  // 虞姬
    expect(HERO_CHAIN[7].heroId).toBe(106);  // 小乔
    expect(HERO_CHAIN[8].heroId).toBe(184);  // 蔡文姬
    expect(HERO_CHAIN[9].heroId).toBe(505);  // 瑶
  });

  it('getHeroImagePath generates correct filenames', () => {
    expect(getHeroImagePath(1, 'marco_polo')).toBe('heroes/01-marcopolo');
    expect(getHeroImagePath(10, 'yao')).toBe('heroes/10-yao');
  });
});
